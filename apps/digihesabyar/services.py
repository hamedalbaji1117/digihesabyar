...
from .models import InvoiceFile, InvoiceRow, PricingTier
    ...
    try:
        ...
        with transaction.atomic():
            ...
            for pr in parsed_rows:
                # تعیین is_return براساس مبلغ فروش:
                ...
                # مالیات فعلاً فقط براساس کمیسیون + نصف هزینه پردازش است (ساده شده)
                tax_amount = (pr.commission_amount + pr.processing_fee) // 10
                ...
                InvoiceRow.objects.create(
                    ...
                    shipping_fee=pr.shipping_fee,
                    processing_fee=pr.processing_fee,
                    platform_dev_revenue=pr.platform_dev_revenue,
            ...
            invoice.save(
                update_fields=["row_count", "processing_price", "status"]
            )
